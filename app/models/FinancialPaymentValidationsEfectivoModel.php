...
/**
     * Procesa la VALIDACIÓN de un pago ya registrado.
     * PROTECCIÓN: Se usa 'observation' para el arqueo, dejando 'payment_metadata' intacto.
     */
    public function processCashPayment(int $paymentId, float $amount, string $currency, ?array $breakdown, int $adminId): bool
    {
        try {
            ...
            // 1. Buscamos el registro para verificar que existe y obtener sus IDs
            $stmt = $this->db->prepare("SELECT student_id, amount FROM tbl_financial_payments WHERE id = ? FOR UPDATE");
            $stmt->execute([$paymentId]);
            $paymentData = $stmt->fetch(PDO::FETCH_ASSOC);

            if (!$paymentData) throw new Exception("El registro de pago no existe.");

            // 2. Buscamos la relación del estudiante para la cascada
            ...
            // --- 3. PREPARAMOS EL ARQUEO PARA 'observation' ---
            // NO tocamos payment_metadata para nada.
            $arqueoParaObservacion = json_encode([
                'tipo_validacion' => 'ARQUEO_VENTANILLA',
                'arqueo_caja' => [
                    'desglose' => $breakdown,
                    'monto_fisico' => $amount,
            ...
            // Cambiamos payment_metadata por observation
            ...
            $this->db->prepare($sqlUpd)->execute([$arqueoParaObservacion, $adminId, $paymentId]);

            // 5. EJECUTAR CASCADA (Afecta el Ledger)
            ...
            error_log("CASH VALIDATION MODEL ERROR: " . $e->getMessage());
            return false;
        }
    }
