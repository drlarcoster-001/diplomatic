...
class UserSecurityModel {
    ...
    public function getUsersPaginated(array $filters, int $limit, int $offset): array
    {
        $params = [];
        $sql = "SELECT id, first_name, last_name, email, document_id, user_type, role, status, avatar 
                FROM tbl_users WHERE 1=1";

        if (!empty($filters['text'])) {
            $sql .= " AND (first_name LIKE ? OR last_name LIKE ? OR email LIKE ? OR document_id LIKE ?)";
            $search = "%" . $filters['text'] . "%";
    ...
    {
        $params = [];
        $sql = "SELECT COUNT(*) FROM tbl_users WHERE 1=1";
