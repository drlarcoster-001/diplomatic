 ...
 * VERSIÓN: 2.6.0 - Agrega Constancia de Estudios con Horario (ESTUDIOS_HORARIO).
 */
...
final class StudentsCertificatesController extends Controller
{
    ...
    public function generate(): void {
        ...
        try {
            ...
            $this->streamPdfResponse($type, $data, $codigo, $url, false, $offeringId);
    ...
    public function sendEmail(): void {
        ...
        try {
            ...
            $dompdf->loadHtml($this->loadTemplate($type, $data, $qrBase64, $certInfo['code'], $offeringId), 'UTF-8');
            $dompdf->render();
                    ...
                    <tr>
                        <td style='padding: 40px 30px;'>
                            ...
                            <div style='margin: 40px 0; text-align: center;'>
                                <a href='$publicUrl' target='_blank' style='background-color: #0056b3; color: #ffffff; padding: 18px 35px; text-decoration: none; border-radius: 8px; font-weight: bold; font-size: 15px; display: inline-block;'>
                                    DESCARGAR DOCUMENTO
                                </a>
                            </div>
    ...
    private function fetchStudentMasterData(int $offeringId): ?array {
        ...
        $sql  = "SELECT u.*, d.name as diplomado_name, d.total_hours,
                        c.start_date as lapso_inicio, c.end_date as lapso_fin,
                        o.class_start, o.general_modality 
                 FROM tbl_students s
    ...
    private function templateEstudios(array $d, string $header): string {
        ...
        $lapso   = mb_strtoupper(
            $this->getMesEspanol((int)date('m', strtotime($d['lapso_inicio']))) . " " .
            date('Y', strtotime($d['lapso_inicio'])) . " – " .
            $this->getMesEspanol((int)date('m', strtotime($d['lapso_fin']))) . " " .
            date('Y', strtotime($d['lapso_fin']))
        );
        $inicio = date('d', strtotime($d['class_start'])) . " de " .
                  $this->getMesEspanol((int)date('m', strtotime($d['class_start']))) . " de " .
                  date('Y', strtotime($d['class_start']));
        ...
        return "<html><body style='font-family: Times-Roman; font-size: 12pt; text-align: justify; padding: 2cm;'>
                ...
                <p>La modalidad del Diplomado es <strong>{$d['general_modality']}</strong>, con clases teóricas y actividades prácticas desarrolladas en el Hospital Central &ldquo;Dr. Antonio María Pineda&rdquo;.</p>
    ...
    private function templateInscripcion(array $d, string $header): string {
        ...
        return "<html><head><style>body{font-family:Times-Roman;font-size:11pt;padding:0.5cm}.center{text-align:center;font-weight:bold;line-height:1.2}.data-table{width:100%;border-collapse:collapse;border:1.5px solid black;margin-top:10px}.data-table td{border:1px solid black;padding:8px;vertical-align:top}.label{font-size:8.5pt;font-weight:bold;display:block;text-transform:uppercase;color:#444}.val{font-size:11pt;padding-top:2px;font-weight:bold}</style></head><body>
                ...
                <table style='width:100%;border-collapse:collapse;margin-top:40px'>
                    <tr>
                        <td width='45%' style='border-top:1.5px solid black;text-align:center;padding-top:10px'><div style='font-weight:bold'>DR. RAFAEL CAMEJO</div><div style='font-size:9pt'>COORDINADOR GENERAL</div></td>
                        <td width='10%'></td>
                        <td width='45%' style='border-top:1.5px solid black;text-align:center;padding-top:10px'><div style='font-weight:bold'>{$nom}</div><div style='font-size:9pt'>FIRMA DEL PARTICIPANTE</div></td>
                    </tr>
                </table>
