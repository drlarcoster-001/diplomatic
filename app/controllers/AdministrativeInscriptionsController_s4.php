...
if ($currency === 'BS') {
            ...
            $metadata['monto_sistema_usd'] = number_format(round($calculo['monto_usd'], 2), 2, '.', '');
            $metadata['tasa_cambio']       = number_format(round($calculo['tasa'], 2), 2, '.', '');


        } else {
            $metadata['tasa_cambio']       = number_format(1, 2, '.', '');
            $metadata['monto_sistema_usd'] = number_format(round($cleanAmount, 2), 2, '.', '');
        }
        ...
        if (isset($metadata['detalles_transaccion']['monto_nativo'])) {
            $metadata['detalles_transaccion']['monto_nativo'] = number_format(round((float)$metadata['detalles_transaccion']['monto_nativo'], 2), 2, '.', '');
        }
