...
class FinancialCashPagomovilController extends Controller
{
    ...
    /**
     * Carga la vista principal con la tasa más reciente del sistema.
     */
    public function index(): void
    {
        $this->clearOutputBuffer();
        
        // Obtenemos la tasa más nueva (la que configuramos en el modelo)
        $lastRate = $this->model->getLastGlobalRate();

        // Inyectamos la variable 'last_rate' para que el index.php la pinte
...
/**
     ...
     * FIX FINAL: Lectura de moneda inteligente para evitar montos inflados.
     */
        ...
        try {
            ...
            if ($xlsx = SimpleXLSX::parse($_FILES['excelFile']['tmp_name'])) {
                ...
                foreach ($rows as $index => $row) {
                    ...
                    if (is_numeric($val)) {
                        // Si la librería ya lo detecta como número, lo usamos directo.
                        $montoFinal = (float)$val;
                    } else {
                        // Si viene como texto (ej: "33.685,24"), limpiamos con cuidado.
                    ...
                    // Referencia: Convertir notación científica a entero limpio
                    $refRaw = $row[2] ?? '';
                    ...
                    // Teléfono: Solo números y últimos 10 dígitos
                    ...
                    $dataToSave[] = [
                        ...
                        'amount_bs'    => $montoFinal // Guardará 33685.24 correctamente
                    ];
                }
