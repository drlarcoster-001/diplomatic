    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <div class="d-flex gap-2">
            ...
            <button class="btn btn-success rounded-pill px-4 shadow-sm" onclick="document.getElementById('formBasicData').submit();">
                <i class="fas fa-save me-1"></i> Guarda
            </button>
        </div>
    </div>
        ...
        <div class="col-lg-3">
            <div class="card border-0 shadow-sm text-center p-4">
                ...
                <div class="position-relative mx-auto mb-3" style="width: 150px; height: 150px;">
                    ...
                    <button type="button" class="btn btn-sm btn-primary rounded-circle position-absolute bottom-0 end-0 shadow btn-change-photo" style="width:35px; height:35px;">
                ...
                <div class="card-body p-4 bg-light bg-opacity-50">
                    <div class="tab-content" id="expedienteTabsContent">
                        ...
                        <div class="tab-pane fade" id="formacion" role="tabpanel">
                            ...
                            <div class="table-responsive">
                                <table class="table table-sm bg-white border rounded table-hover">
                                    <thead class="bg-light"><tr><th>Título</th><th>Institución</th><th>Año</th><th class="text-end"></th></tr></thead>
                                    <tbody>
                                        ...
                                        <tr class="edit-row" data-target-modal="#modalFormation" data-json='<?= htmlspecialchars(json_encode($f), ENT_QUOTES, 'UTF-8') ?>' style="cursor:pointer;">
                                            ...
                                            <td class="text-end">
                                                <form action="/diplomatic/public/academic/profesores/deleteFormation" method="POST" class="d-inline">
                                                    ...
                                                    <button type="button" class="btn btn-sm text-danger btn-delete-record"><i class="fas fa-trash"></i></button>
                                                </form>
                                            </td>
                                        </tr>
                                        <?php endforeach; ?>
                        ...
                        <div class="tab-pane fade" id="documentos" role="tabpanel">
                            ...
                            <div class="table-responsive">
                                <table class="table table-sm bg-white border rounded table-hover">
                                    <thead class="bg-light"><tr><th>Tipo</th><th>Nombre</th><th>Archivo</th><th class="text-end"></th></tr></thead>
                                    <tbody>
                                        <?php foreach($p['documents'] as $d): ?>
                                        <tr class="edit-row" data-target-modal="#modalDocument" data-json='<?= htmlspecialchars(json_encode($d), ENT_QUOTES, 'UTF-8') ?>' style="cursor:pointer;">
                                            <td><?= htmlspecialchars($d['document_type']) ?></td>
                                            <td><?= htmlspecialchars($d['document_name']) ?></td>
                                            <td><a href="<?= $d['file_path'] ?>" target="_blank" class="text-primary no-edit small">Ver</a></td>
                                            <td class="text-end">
...
<div class="modal fade" id="modalCrop" tabindex="-1" aria-hidden="true"><div class="modal-dialog modal-dialog-centered"><div class="modal-content"><div class="modal-header bg-dark text-white"><h5>Ajustar Foto</h5></div><div class="modal-body p-0"><img id="imageToCrop" src="" style="max-width: 100%;"></div><div class="modal-footer"><button type="button" class="btn btn-secondary rounded-pill" data-bs-dismiss="modal">Cerrar</button><button type="button" class="btn btn-primary rounded-pill" id="btnSaveCrop">Guarda</button></div></div></div></div>

<div class="modal fade" id="modalFormation" tabindex="-1" aria-hidden="true"><div class="modal-dialog"><form action="/diplomatic/public/academic/profesores/saveFormation" method="POST" class="modal-content"><div class="modal-header bg-primary text-white"><h5 class="modal-title">Formación Académica</h5></div><div class="modal-body p-4"><input type="hidden" name="id"><input type="hidden" name="professor_id" value="<?= $p['id'] ?>"><div class="mb-3"><label class="small fw-bold">TÍTULO</label><input type="text" name="degree_title" class="form-control" required></div><div class="mb-3"><label class="small fw-bold">INSTITUCIÓN</label><input type="text" name="institution" class="form-control" required></div><div class="mb-3"><label class="small fw-bold">AÑO</label><input type="number" name="year_obtained" class="form-control"></div></div><div class="modal-footer"><button type="button" class="btn btn-secondary rounded-pill" data-bs-dismiss="modal">Cerrar</button><button type="submit" class="btn btn-primary rounded-pill btn-submit-modal">Guarda</button></div></form></div></div>

<div class="modal fade" id="modalWork" tabindex="-1" aria-hidden="true"><div class="modal-dialog"><form action="/diplomatic/public/academic/profesores/saveWork" method="POST" class="modal-content"><div class="modal-header bg-primary text-white"><h5 class="modal-title">Experiencia Laboral</h5></div><div class="modal-body p-4"><input type="hidden" name="id"><input type="hidden" name="professor_id" value="<?= $p['id'] ?>"><div class="mb-3"><label class="small fw-bold">CARGO</label><input type="text" name="job_title" class="form-control" required></div><div class="mb-3"><label class="small fw-bold">EMPRESA</label><input type="text" name="institution" class="form-control" required></div>
    
    <div class="form-check mb-3"><input class="form-check-input" type="checkbox" name="is_current" id="check_current" value="1"><label class="form-check-label small fw-bold" for="check_current">Cargo Actual</label></div>
    
    <div class="row g-3"><div class="col-6"><label class="small fw-bold">INICIO</label><input type="date" name="start_date" id="work_start_date" class="form-control" required></div><div class="col-6"><label class="small fw-bold">FIN</label><input type="date" name="end_date" id="work_end_date" class="form-control"></div></div></div>
    
    <div class="modal-footer"><button type="button" class="btn btn-secondary rounded-pill" data-bs-dismiss="modal">Cerrar</button><button type="submit" class="btn btn-primary rounded-pill btn-submit-modal">Guarda</button></div></form></div></div>

<div class="modal fade" id="modalSpecialty" tabindex="-1" aria-hidden="true"><div class="modal-dialog"><form action="/diplomatic/public/academic/profesores/saveSpecialty" method="POST" class="modal-content"><div class="modal-header bg-primary text-white"><h5 class="modal-title">Especialidad</h5></div><div class="modal-body p-4"><input type="hidden" name="id"><input type="hidden" name="professor_id" value="<?= $p['id'] ?>"><div class="mb-3"><label class="small fw-bold">NOMBRE</label><input type="text" name="specialty_name" class="form-control" required></div><div class="form-check form-switch"><input class="form-check-input" type="checkbox" name="is_main" value="1" id="isMainCheck"><label class="form-check-label small fw-bold">Principal</label></div></div><div class="modal-footer"><button type="button" class="btn btn-secondary rounded-pill" data-bs-dismiss="modal">Cerrar</button><button type="submit" class="btn btn-primary rounded-pill btn-submit-modal">Guarda</button></div></form></div></div>

<div class="modal fade" id="modalDocument" tabindex="-1" aria-hidden="true"><div class="modal-dialog"><form action="/diplomatic/public/academic/profesores/uploadDocument" method="POST" enctype="multipart/form-data" class="modal-content"><div class="modal-header bg-primary text-white"><h5 class="modal-title">Documento</h5></div><div class="modal-body p-4"><input type="hidden" name="id"><input type="hidden" name="professor_id" value="<?= $p['id'] ?>"><div class="mb-3"><label class="small fw-bold">TIPO</label><select name="document_type" class="form-select"><option>CV</option><option>Identidad</option><option>Certificación</option></select></div><div class="mb-3"><label class="small fw-bold">NOMBRE</label><input type="text" name="document_name" class="form-control" required></div><div class="mb-3" id="file_input_wrapper"><label class="small fw-bold">ARCHIVO</label><input type="file" name="document_file" class="form-control"></div></div><div class="modal-footer"><button type="button" class="btn btn-secondary rounded-pill" data-bs-dismiss="modal">Cerrar</button><button type="submit" class="btn btn-primary rounded-pill btn-submit-modal">Guarda</button></div></form></div></div>

<script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/cropperjs/1.5.13/cropper.min.js"></script>
<script src="/diplomatic/public/assets/js/academic_profesores_edit.js?v=<?= time() ?>"></script>