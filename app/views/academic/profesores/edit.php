                    ...
                    <div class="tab-content">
                        <div class="tab-pane fade show active" id="datos">
                            <form id="formBasicData" action="/diplomatic/public/academic/profesores/updateBase" method="POST" class="row g-4">
                                ...
                                <div class="col-md-4"><label class="form-label small fw-bold">IDENTIFICACIÓN</label><input type="text" name="identification" class="form-control" value="<?= $p['identification'] ?>" required></div>
                                <div class="col-md-4"><label class="form-label small fw-bold">NOMBRES</label><input type="text" name="first_name" class="form-control" value="<?= $p['first_name'] ?>" required></div>
                                <div class="col-md-4"><label class="form-label small fw-bold">APELLIDOS</label><input type="text" name="last_name" class="form-control" value="<?= $p['last_name'] ?>" required></div>
                                <div class="col-md-4"><label class="form-label small fw-bold">EMAIL</label><input type="email" name="contact_email" class="form-control" value="<?= $p['contact']['email'] ?? '' ?>"></div>
                                <div class="col-md-4"><label class="form-label small fw-bold">TELÉFONO</label><input type="text" name="contact_phone" class="form-control" value="<?= $p['contact']['phone'] ?? '' ?>"></div>
                                <div class="col-md-4"><label class="form-label small fw-bold">LINKEDIN</label><input type="url" name="contact_linkedin" class="form-control" value="<?= $p['contact']['linkedin_url'] ?? '' ?>"></div>
                                <div class="col-md-6"><label class="form-label small fw-bold">OTRO CONTACTO</label><input type="text" name="other_contact" class="form-control" value="<?= $p['contact']['other_contact'] ?? '' ?>"></div>
                        ...
                        <div class="tab-pane fade" id="formacion">
                            <div class="d-flex justify-content-between mb-3 align-items-center"><h6 class="fw-bold mb-0">Formación Académica</h6><button class="btn btn-sm btn-primary rounded-pill px-3 btn-add-formation">+ Añadir</button></div>
                            <table class="table table-sm bg-white border rounded">
                                ...
                                <tbody>
                                    ...
                                    <tr><td><?= htmlspecialchars($f['degree_title']) ?></td><td><?= htmlspecialchars($f['study_area'] ?? '-') ?></td><td><?= htmlspecialchars($f['institution']) ?></td><td><?= $f['year_obtained'] ?></td><td class="text-end"><form action="/diplomatic/public/academic/profesores/deleteFormation" method="POST"><input type="hidden" name="id" value="<?= $f['id'] ?>"><input type="hidden" name="professor_id" value="<?= $p['id'] ?>"><button type="button" class="btn btn-sm text-danger btn-delete-record"><i class="fas fa-trash"></i></button></form></td></tr>
                                    <?php endforeach; ?>
                                </tbody>
                            </table>
                        </div>
                        <div class="tab-pane fade" id="experiencia">
                            <div class="d-flex justify-content-between mb-3 align-items-center"><h6 class="fw-bold mb-0">Trayectoria Laboral</h6><button class="btn btn-sm btn-primary rounded-pill px-3 btn-add-work">+ Añadir</button></div>
                            <table class="table table-sm bg-white border rounded">
                                ...
                                <tbody>
                                    ...
                                    <tr><td><?= htmlspecialchars($w['job_title']) ?></td><td><?= htmlspecialchars($w['institution']) ?></td><td><?= $w['start_date'] ?> / <?= $w['is_current'] ? 'Hoy' : $w['end_date'] ?></td><td class="text-end"><form action="/diplomatic/public/academic/profesores/deleteWork" method="POST"><input type="hidden" name="id" value="<?= $w['id'] ?>"><input type="hidden" name="professor_id" value="<?= $p['id'] ?>"><button type="button" class="btn btn-sm text-danger btn-delete-record"><i class="fas fa-trash"></i></button></form></td></tr>
                                    <?php endforeach; ?>
                                </tbody>
                            </table>
                        </div>
                        <div class="tab-pane fade" id="especialidades">
                            <div class="d-flex justify-content-between mb-3 align-items-center"><h6 class="fw-bold mb-0">Áreas de Conocimiento</h6><button class="btn btn-sm btn-primary rounded-pill px-3 btn-add-specialty">+ Añadir</button></div>
                            <table class="table table-sm bg-white border rounded">
                                <thead class="bg-light"><tr><th>Especialidad</th><th>Principal</th><th class="text-end"></th></tr></thead>
                                <tbody>
                                    <?php foreach($p['specialties'] as $s): ?>
                                    <tr><td><?= htmlspecialchars($s['specialty_name']) ?></td><td><?= $s['is_main'] ? 'SÍ' : 'NO' ?></td><td class="text-end"><form action="/diplomatic/public/academic/profesores/deleteSpecialty" method="POST"><input type="hidden" name="id" value="<?= $s['id'] ?>"><input type="hidden" name="professor_id" value="<?= $p['id'] ?>"><button type="button" class="btn btn-sm text-danger btn-delete-record"><i class="fas fa-trash"></i></button></form></td></tr>
                                    <?php endforeach; ?>
                                </tbody>
                            </table>
                        </div>
                        <div class="tab-pane fade" id="documentos">
                            <div class="d-flex justify-content-between mb-3 align-items-center"><h6 class="fw-bold mb-0">Soportes Digitales</h6><button class="btn btn-sm btn-primary rounded-pill px-3 btn-add-document">+ Subir</button></div>
                            <table class="table table-sm bg-white border rounded">
                                <thead class="bg-light"><tr><th>Nombre</th><th>Ver</th><th class="text-end"></th></tr></thead>
                                <tbody>
                                    <?php foreach($p['documents'] as $d): ?>
                                    <tr><td><?= htmlspecialchars($d['document_name']) ?></td><td><a href="<?= $d['file_path'] ?>" target="_blank" class="btn btn-link text-primary px-0">Ver Archivo</a></td><td class="text-end"><form action="/diplomatic/public/academic/profesores/deleteDocument" method="POST"><input type="hidden" name="id" value="<?= $d['id'] ?>"><input type="hidden" name="professor_id" value="<?= $p['id'] ?>"><button type="button" class="btn btn-sm text-danger btn-delete-record"><i class="fas fa-trash"></i></button></form></td></tr>
                                    <?php endforeach; ?>
                                </tbody>
                            </table>
                        </div>
