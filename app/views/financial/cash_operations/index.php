 ...
 * ARCHIVO: app/views/financial/cash_operations/index.php
 * PROPÓSITO: Panel de validación con tarjetas horizontales.
 * VERSIÓN: 1.3.3 - FIX: Sincronización de rutas y prevención de caché JS.
 */

declare(strict_types=1);

// Detectar base path dinámicamente para que funcione en cualquier PC del equipo
$basePath = rtrim(str_replace('\\', '/', dirname($_SERVER['SCRIPT_NAME'] ?? '')), '/');
$counts = $data['counts'] ?? [];
?>

<link rel="stylesheet" href="<?= $basePath ?>/assets/css/financial_cash_operations.css">
...
<div class="container-fluid py-4 animate__animated animate__fadeIn">
    ...
    <div class="row justify-content-center">
        <div class="col-12" style="max-width: 900px;">
            ...
            <div class="d-flex justify-content-between align-items-center mb-4">
                <div>
                    <h4 class="fw-bold text-dark mb-1">
                        <i class="bi bi-shield-check text-primary me-2"></i> Validación de Inscripciones
                    </h4>
                    <p class="text-muted small">Seleccione un método de pago para verificar los reportes de nuevos ingresos.</p>
                </div>
            ...
            <div class="card border-0 shadow-sm rounded-4 mb-3 card-financial-option" data-route="pagomovil" style="cursor:pointer;">
                <div class="card-body p-4 d-flex align-items-center border-start border-4 border-primary rounded-start-4 bg-white">
                    ...
                    <div class="flex-grow-1">
                        <h5 class="fw-bold text-dark mb-1">Pago Móvil (Bs.)</h5>
                        <p class="text-muted small mb-0">Conciliación de transferencias nacionales y referencias bancarias.</p>
                    </div>
                    <div class="ms-3 d-flex align-items-center">
                        <?php $c = $counts['PAGOMOVIL'] ?? 0; ?>
                        <span class="badge bg-danger rounded-pill px-3 py-2 me-3 shadow-sm <?= $c > 0 ? 'animate__animated animate__headShake' : 'd-none' ?>">
                            <?= $c ?> Pendiente<?= $c !== 1 ? 's' : '' ?>
                        </span>
            ...
            <div class="card border-0 shadow-sm rounded-4 mb-3 card-financial-option" data-route="zelle" style="cursor:pointer;">
                <div class="card-body p-4 d-flex align-items-center border-start border-4 border-info rounded-start-4 bg-white">
                    ...
                    <div class="flex-grow-1">
                        <h5 class="fw-bold text-dark mb-1">Zelle (USD)</h5>
                        <p class="text-muted small mb-0">Validación de transferencias directas internacionales en divisas.</p>
                    </div>
                    <div class="ms-3 d-flex align-items-center">
                        <?php $c = $counts['ZELLE'] ?? 0; ?>
                        <span class="badge bg-danger rounded-pill px-3 py-2 me-3 shadow-sm <?= $c > 0 ? 'animate__animated animate__headShake' : 'd-none' ?>">
                            <?= $c ?> Pendiente<?= $c !== 1 ? 's' : '' ?>
                        </span>
            ...
            <div class="card border-0 shadow-sm rounded-4 mb-3 card-financial-option" data-route="binance" style="cursor:pointer;">
                <div class="card-body p-4 d-flex align-items-center border-start border-4 border-warning rounded-start-4 bg-white">
                    ...
                    <div class="flex-grow-1">
                        <h5 class="fw-bold text-dark mb-1">Binance (USDT)</h5>
                        <p class="text-muted small mb-0">Confirmación de pagos criptográficos mediante TXID o QR.</p>
                    </div>
                    <div class="ms-3 d-flex align-items-center">
                        <?php $c = $counts['BINANCE'] ?? 0; ?>
                        <span class="badge bg-danger rounded-pill px-3 py-2 me-3 shadow-sm <?= $c > 0 ? 'animate__animated animate__headShake' : 'd-none' ?>">
                            <?= $c ?> Pendiente<?= $c !== 1 ? 's' : '' ?>
                        </span>
            ...
            <div class="card border-0 shadow-sm rounded-4 mb-3 card-financial-option" data-route="efectivo" style="cursor:pointer;">
                <div class="card-body p-4 d-flex align-items-center border-start border-4 border-success rounded-start-4 bg-white">
                    ...
                    <div class="flex-grow-1">
                        ...
                        <p class="text-muted small mb-0">Registro y arqueo de pagos presenciales en oficina.</p>
                    </div>
                    <div class="ms-3 d-flex align-items-center">
                        <?php $c = $counts['EFECTIVO'] ?? 0; ?>
                        <span class="badge bg-danger rounded-pill px-3 py-2 me-3 shadow-sm <?= $c > 0 ? 'animate__animated animate__headShake' : 'd-none' ?>">
                            <?= $c ?> Pendiente<?= $c !== 1 ? 's' : '' ?>
                        </span>
...
<script> 
    // BASE_URL se construye dinámicamente para el JS
    const BASE_URL = '<?= $basePath ?>'; 
</script>
<script src="<?= $basePath ?>/assets/js/financial_cash_operations.js?v=<?= time() ?>"></script>