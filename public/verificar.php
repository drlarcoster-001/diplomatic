<?php
/**
 * MÓDULO: GESTIÓN ADMINISTRATIVA / VERIFICACIÓN
 * ARCHIVO: public/verificar.php
 * PROPÓSITO: Validador público dual (Inscripción/Estudios). Confirma la existencia del registro en BD sin visor de archivos.
 * VERSIÓN: 1.7.0 - UI Dual Dinámica + Eliminación de visor PDF + Blindaje Institucional UCLA.
 */

// 1. Cargamos el motor de búsqueda privado (Bypass de Middleware)
require_once __DIR__ . '/../app/services/CertificateValidator.php';

// 2. Captura y sanitización del código
$code = filter_input(INPUT_GET, 'code', FILTER_SANITIZE_SPECIAL_CHARS);
$cert = $code ? CertificateValidator::validate($code) : null;

// 3. Configuración de Interfaz Dinámica según el tipo de documento
$isEstudios = ($cert && $cert['type'] === 'ESTUDIOS');
$themeColor = $isEstudios ? '#003366' : '#006666'; // Azul para Estudios, Verde Oscuro para Inscripción
$instName   = $isEstudios ? 'CONSTANCIA DE ESTUDIOS' : 'PLANILLA DE INSCRIPCIÓN';
?>
<!DOCTYPE html>
<html lang="es">
<head>
    ...
    <style>
        :root { --ucla-blue: #003366; --ucla-gold: #ffb81c; --brand-color: <?= $themeColor ?>; }
...
<div class="container px-3">
    <div class="card main-card">
        <div class="header">
            ...
            <h5 class="mb-0 fw-bold"><?= $instName ?></h5>
        ...
        <div class="card-body p-4 text-center">
            <?php if ($cert): ?>
                ...
                <div class="info-panel shadow-sm">
                    ...
                    <div class="data-value"><?= htmlspecialchars(strtoupper($cert['first_name'] . ' ' . $cert['last_name'])) ?></div>

                    <div class="data-label">Programa Académico:</div>
                    <div class="data-value"><?= htmlspecialchars($cert['diplomado']) ?></div>

                    <div class="data-label">Folio de Registro:</div>
                    <div class="data-value text-primary"><?= htmlspecialchars($cert['code']) ?></div>

                    <div class="data-label">Estatus en Plataforma:</div>
                    <div class="data-value mb-0">
                        <span class="badge bg-primary px-3">
                            <?= ($cert['type'] === 'INSCRIPCION') ? 'REGISTRO FORMALIZADO' : 'ALUMNO REGULAR (CURSANDO)' ?>
                        </span>
                    </div>
                </div>
            ...
            <?php else: ?>
                <div class="py-5">
                    ...
                    <p class="text-muted px-4">El código de folio consultado no existe o no corresponde a un documento oficial emitido por la institución.</p>
                    <hr class="mx-5">
            ...
            <div class="mt-4 pt-3 border-top">
                <small class="text-muted">
                    Decanato de Ciencias de la Salud &bull; <?= date('d/m/Y') ?>
                </small>
            </div>
        </div>
    </div>
