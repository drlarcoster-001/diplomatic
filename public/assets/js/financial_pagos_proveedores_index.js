 ...
 * ARCHIVO: public/assets/js/financial_pagos_proveedores_index.js
 * PROPÓSITO: Acciones de descartar y reversar desde el listado.
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('.btn-descartar-pago').forEach(btn => {
        btn.addEventListener('click', async () => {
            const confirmed = await Swal.fire({
                title: '¿Descartar este pago?',
                html: `Se eliminará <strong>${btn.dataset.numero}</strong> y todos sus ítems. Esta acción no se puede deshacer.`,
                icon: 'warning',
    ...
    document.querySelectorAll('.btn-reversar-pago').forEach(btn => {
        btn.addEventListener('click', async () => {
            const confirmed = await Swal.fire({
                title: '¿Reversar este pago?',
                html: `<strong>${btn.dataset.numero}</strong> retrocederá un paso en el flujo.`,
                icon: 'question',
                showCancelButton: true,
