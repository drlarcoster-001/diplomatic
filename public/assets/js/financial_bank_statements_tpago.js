 ...
 * ARCHIVO: public/assets/js/financial_bank_statements_tpago.js
 * PROPÓSITO: Lógica de administración y carga de transacciones T-Pago.
 *            Grid paginado con filtros por fecha, referencia, monto y teléfono.
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    console.log("Módulo Bank Statements T-Pago v1.0.0: Operativo.");
    ...
    function initFilters() {
        ...
        // Enter en cualquier input de filtro
        ['filter-date', 'filter-reference', 'filter-amount', 'filter-phone'].forEach(id => {
    ...
    function applyFilters() {
        ...
        state.filterPhone     = document.getElementById('filter-phone').value.trim();
        state.currentPage     = 1;
        fetchTransactions();
    }
    ...
    async function fetchTransactions() {
        const tbody = document.querySelector('#table-tpago tbody');
        if (!tbody) return;
        ...
        try {
            const params = new URLSearchParams({
                ...
                phone:     state.filterPhone,
            });

            const res    = await fetch(`${BASE_URL}/financial/bank_statements/tpago/getTransactions?${params}`);
            ...
            result.data.forEach((row, i) => {
                ...
                <tr class="align-middle">
                    ...
                    <td><span class="badge bg-success bg-opacity-10 text-success fw-bold px-2">${row.op_type}</span></td>
                    <td class="font-monospace small">${row.op_date}</td>
                    <td class="font-monospace text-primary fw-bold small">${row.reference_id}</td>
                    <td class="small">${row.origin_phone || '-'}</td>
                    <td class="small text-muted">${row.origin_bank || '-'}</td>
            ...
            console.error("Error fetch T-Pago:", e);
            tbody.innerHTML = `
                <tr>
    ...
    function initModalExcel() {
        ...
        const dropzone   = document.getElementById('dropzone-tpago');
        ...
        btnProcess.addEventListener('click', async () => {
            ...
            Swal.fire({
                ...
                text: 'Sincronizando transacciones T-Pago...',
                allowOutsideClick: false,
                didOpen: () => Swal.showLoading()
            });

            try {
                const res    = await fetch(`${BASE_URL}/financial/bank_statements/tpago/uploadFile`, { method: 'POST', body: fd });
                const result = await res.json();
